	...
	$("#appraiseBtn").click(function() {
		...
		$.ajax({
			...
			success: function(data) {
				if (data.retCode == 1) {
					if ($("#myType").val() == "myAppraise") {
						layer.msg(data.retMsg, {
							time: 4000
						});
						window.location.href = ctx + "/student_user.html?type=appr&coachType=myAppraise";
					} else {
						layer.msg(data.retMsg, {
							time: 4000
						});
						window.location.href = ctx + "/student_user.html?type=train";
					}
				} else {
					layer.msg(data.retMsg);
				}
